...
const handleError = (error, options = {}) => {
  const requestLogger = Object.hasOwn(options, 'requestLogger')
  ...
  return { request, requestLogger, response, next };
};
...
describe('errorHandler', () => {
  ...
  it('logs one metadata-only event for an unknown HTTP 500 error', () => {
    const mongoUri =
      'mongodb+srv://sensitive-user:sensitive-password@cluster.example.mongodb.net/database';
    const cause = new Error(`Private cause containing ${mongoUri}`);
    const error = new Error(`Private unknown message containing ${mongoUri}`, { cause });
    error.stack = `Private stack containing ${mongoUri}`;

    const { request, requestLogger } = handleError(error);

    expect(requestLogger.error).toHaveBeenCalledOnce();
    ...
    const serializedLogArguments = JSON.stringify(requestLogger.error.mock.calls);
    expect(serializedLogArguments).not.toContain(error.message);
    expect(serializedLogArguments).not.toContain(error.stack);
    expect(serializedLogArguments).not.toContain(cause.message);
    expect(serializedLogArguments).not.toContain(mongoUri);
    expect(serializedLogArguments).not.toContain(request.headers.authorization);
    expect(serializedLogArguments).not.toContain(request.cookies.session);
  });

  it('falls back to the root logger when request.log is unavailable', () => {
    mocks.rootLogger.error.mockClear();
    const error = new Error('Internal failure.');
    const requestLogger = null;

    handleError(error, { requestLogger });

    expect(mocks.rootLogger.error).toHaveBeenCalledOnce();
  });

  it('does not log validation details or expected client errors separately', () => {
    const details = [
      {
        field: 'body.password',
        code: 'TOO_SMALL',
    ...
    const { requestLogger } = handleError(error);
